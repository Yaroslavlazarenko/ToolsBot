...
class FunctionHandler:
    ...
    async def analyze_video_content(self, text_from_router: str) -> str:
        ...
        try:
            ...
            video_segments_paths = await asyncio.to_thread(cut_video_to_segments, original_video_path, segment_time=600, output_dir=segments_dir)
            tasks = [asyncio.create_task(self._process_video_segment(path, i + 1, len(video_segments_paths), text_from_router)) for i, path in enumerate(video_segments_paths)]
    ...
    async def _process_video_segment(self, segment_path: str, index: int, total: int, user_prompt: str) -> Optional[str]:
        uploaded_file = None
        try:
            ...
            prompt = f"""
            This is segment {index} of {total} from a large video. Analyze it and provide a description of what is happening on the video track.
